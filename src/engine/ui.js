...
export function updateAll(){ updateHUD();updateHand();updateRelics(); }
...
export function createCardEl(card,idx){
  ...
  el.innerHTML='<div class="card-cost" style="background:'+costColor+'">'+card.cost+'</div><div class="card-name">'+card.name+'</div><div class="card-effect">'+card.effect+'</div><div class="card-type '+typeCls+'">'+typeLabel+'</div>';
  return el;
}
...
export function updateEnemyDisplay(makeCtx){
  ...
  document.getElementById("enemy-name").textContent=s.ene.name+poisonStr;
